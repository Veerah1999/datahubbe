...
def put_play(mig_id):
        qset=schema_migration.objects.get(id=mig_id)
        qset.Play=(False)
        qset.save()
        return Response(qset)
...
def schema_teradata(sources):
        ...
        try:
                ...
                print("Connection successful")
                sql = f"""select TableName,DataBaseName,ColumnName,ColumnLength,ColumnType,ColumnId,CreatorName 
...
def source_postgres(sources,table_name):
        try:
                ...
                while True:
                        ...
                        json_credentials_path = r"C:\Users\Administrator\Downloads\gcp new key.json"
...
def source_mysql(sources,table_name):
        try:
                ...
                while True:
                        ...
                        reader = pa.BufferReader(parquet_file.getbuffer())
                        json_credentials_path = r"C:\Users\Administrator\Downloads\gcp new key.json"
                        bucket = storage.client.bucket("databucket8806")
                        blob = bucket.blob(f"{table_name}_{offset}.parquet")
                        # blob.upload_from_filename('gcp_url')
                        blob.upload_from_file(reader, content_type='application/octet-stream')
                        print(f'Upload complete for gcs')
...
#                 temp_tarconnection_name.update(i)
#         # schedule id posting
#         post_schema= schema_log(config_id=temp_config['id'],schema_id=temp_migration['id'],status="running",start_date=today,start_time=time,)        
#         post_schema.save()
#         # getting run id
...
#                         # Use BufferReader instead of io.BytesIO to read the Parquet file
#                         reader = pa.BufferReader(parquet_file.getbuffer())
#                         json_credentials_path = r"C:\Users\Administrator\PycharmProjects\pythonProject\blissful-mantis-375706-26b7057ec4f6.json"
#                         client = storage.Client.from_service_account_json(json_credentials_path=json_credentials_path)
...
#                 print(f'Migrating data from sf {table_name}...')
#                 cur = url.cursor()
#                 cur.execute("""CREATE or replace STORAGE INTEGRATION gcs_inter
#                                 TYPE = EXTERNAL_STAGE
#                                 STORAGE_PROVIDER = 'GCS'
...
#                 while True:
#                         j = offset // chunksize
#                         format_name = f"{table_name}_format_{j}"
#                         cur.execute(f"""CREATE OR REPLACE FILE FORMAT {format_name}
#                                                                 TYPE = 'PARQUET'""")

#                         # Create a stage for the current chunk
#                         chunk_stage_name = f"{table_name}_chunk_{j}"
#                         url = f'gcs://data4423/{table_name}_{j * chunksize}.parquet'
#                         cur.execute(f"""CREATE OR REPLACE STAGE {chunk_stage_name}
#                                                                 URL = '{url}'
#                                                                 storage_integration = gcs_inter
#                                                                 FILE_FORMAT = (FORMAT_NAME = {format_name})""")
...
#                         # Load data from the current stage into the table
#                         cur.execute(f"""COPY INTO {table_name} FROM @{chunk_stage_name}
#                                 MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE
#                                 on_error = 'skip_file';""")
...
#                         cur.execute(f"DROP FILE FORMAT {format_name}")
#                         cur.execute(f"DROP STAGE {chunk_stage_name}")
#                         print(f'data loaded to {table_name} for offset {j}')

#                         # Check if there is more data
#                         json_credentials_path = r"C:\Users\Administrator\PycharmProjects\pythonProject\blissful-mantis-375706-26b7057ec4f6.json"
#                         client = storage.Client.from_service_account_json(json_credentials_path=json_credentials_path)
#                         bucket = client.bucket("data4423")
#                         blobs = bucket.list_blobs(prefix=f"{table_name}_{offset}.parquet")
#                         if len(list(blobs)) == j + 1:
#                                 break
#                         # Increment the offset
#                         offset += chunksize

#                 print(f"All data for table {table_name} is uploaded.")
