...
class TableauConnector:
    ...
    def download_workbook_as_pdf(self, workbook_name):
        ...
        with open(rf'/home/ubuntu/datahub-v3/datahub_v3_project/media/sample_report.pdf', 'wb') as file:
...
class PDFDownloader(APIView):
    def post(self, request):
        ...
        try:
            ...
            #Get the file path of the downloaded PDF
            file_path = os.path.join(settings.MEDIA_ROOT, 'sample_report.pdf')

            # Generate the URL path to access the downloaded file
            file_url = request.build_absolute_uri(settings.MEDIA_URL + 'sample_report.pdf')

            return Response({'message': 'Tableau workbook as a PDF file is downloaded successfully', 'file_url': file_url}, status=status.HTTP_200_OK)
